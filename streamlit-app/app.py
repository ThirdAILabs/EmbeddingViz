...
# patht to the training file
data_path = "processed_recipes_3.csv"
# key Query string for model
QUERY = 'Name'
...
# this needs to be changed as per model query needs to be preprocessed
def process_query(query):
    query = query.replace('\n', ' ').strip()
    query = ' '.join([query[i :i + 4] for i in range(len(query) - 3)])
    return query

def get_embeddings_and_legend(user_input, topn=5):
    legend = []
    temp = []
    model = load_model()
    data = load_data()
    print(user_input, topn)
    for k in range(len(user_input)) :
        query = user_input[k].lower()
        query = process_query(query)
        cashew = model.embedding_representation({QUERY : query})
        scores = model.predict({QUERY : query.lower()})
        sorted_ids = scores.argsort()[-topn :][: :-1]
        temp.append(cashew)
        legend.append(user_input[k])
        for id in sorted_ids :
            temp.append(model.get_entity_embedding(int(model.class_name(id))))
            legend.append(data.iloc[id][QUERY])
