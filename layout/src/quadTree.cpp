...
//  https://github.com/anvaka/ngraph.quadtreebh3d/blob/master/index.js Some of
//  the coments are no longer relvant to C++.
//
...
QuadTreeNode *QuadTree::createRootNode(std::vector<Body> &bodies) {
  double x1 = INT32_MAX, x2 = INT32_MIN, y1 = INT32_MAX, y2 = INT32_MIN,
...
void QuadTree::insert(Body *body, QuadTreeNode *node) {
  if (!node->body) {
    // This is internal node. Update the total mass of the node and
    // center-of-mass.
    double x = body->pos.x;
    double y = body->pos.y;
    double z = body->pos.z;
    ...
    double left = node->left, right = (node->right + left) / 2, top = node->top,
           bottom = (node->bottom + top) / 2, back = node->back,
    ...
    if (oldBody->pos.sameAs(body->pos)) {
      ...
      do {
        ...
        retriesCount -= 1;
        // Make sure we don't bump it out of the box. If we do, next iteration
        // should fix it
...
void QuadTree::updateBodyForce(Body *sourceBody) {
  ...
  while (queueLength) {
    ...
    if (body != NULL && differentBody) {
      ...
      // Otherwise, calculate the ratio s / r,  where s is the width of the
      // region represented by the internal node, and r is the distance between
      // the body and the node's center-of-mass
      ...
      // If s / r < θ, treat this internal node as a single body, and calculate
      // the force it exerts on sourceBody, and add this amount to sourceBody's
      // net force.
      if ((node->right - node->left) / r < layoutSettings->theta) {
        // in the if statement above we consider node's width only
        // because the region was squarified during tree creation.
        // Thus there is no difference between using width or height.
        v = layoutSettings->gravity * node->mass * sourceBody->mass /
            (r * r * r);
        fx += v * dx;
        fy += v * dy;
        fz += v * dz;
