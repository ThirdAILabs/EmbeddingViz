...
void Layout::init(int *links, long linksSize, int *initialPositions,
                  size_t posSize) {
  initBodies(links, linksSize);
  if (bodies.size() * 3 != posSize) {
    cout << "There are " << bodies.size() << " nodes in the graph and " << endl
         << posSize << " positions. It is expected that each body has exactly"
         << endl
         << "3 Int32 records in the positions file (x, y, z). However this is "
            "not the case"
         << endl
         << "here. Are you sure you are loading correct positions for this "
            "graph file?";
    ...
    for (size_t j = 0; j < body->springs.size(); ++j) {
      if (!bodies[body->springs[j]].positionInitialized()) {
        Vector3 neighbourPosition(
            sourcePos->x + random.next(settings.springLength) -
                settings.springLength / 2,
            sourcePos->y + random.next(settings.springLength) -
                settings.springLength / 2,
            sourcePos->z + random.next(settings.springLength) -
                settings.springLength / 2);
...
size_t Layout::getBodiesCount() { return bodies.size(); }
...
double Layout::integrate() {
  double dx = 0, tx = 0, dy = 0, ty = 0, dz = 0, tz = 0,
...
#pragma omp parallel for reduction(+ : tx, ty, tz) private(dx, dy, dz)
  for (size_t i = 0; i < bodies.size(); i++) {
    ...
    double vx = body->velocity.x, vy = body->velocity.y, vz = body->velocity.z,
